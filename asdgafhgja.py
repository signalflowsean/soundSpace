...
def audio_loop():
    ...
    while True:
        ...
        total = 0
        old_freqs = freqs[:]
        old_mults = [f / fs * 2 * np.pi for f in old_freqs]
        olf_offsets = offsets[:]
        for i, (az, alt, dist) in enumerate(polars):
        #i = 0
        #if not polars:
        ...
        #az, alt, dist = polars[0]
            ...
            old_offset = old_offsets[i] if i < len(old_offsets) else 0
            old_mult = old_mults[i] if i < len(old_mults) else mult
            ...
            for scale, freq_scale in [(pc / 12, 1 / 2), (1 - pc / 12, 1)]:#scale, freq in shep:
                hmm = np.linspace(old_mult, mult, chunk_size, endpoint=False)
                tmp_phase = last_phase * freq_scale
                tmp_offset = tmp_phase - t * hmm * freq_scale
                chunk += scale / 2 * amp * np.sin((t + np.arange(chunk_size)) * hmm * freq_scale + tmp_offset)
                #print('edges', tmp_phase, t * mult * freq_scale + tmp_offset, chunk[0], chunk[-1])
        #old_freq = freq
        #print(total)
        if total > 1:
            chunk /= total
